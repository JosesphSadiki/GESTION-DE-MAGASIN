...
def Enregistrer_vente(LISTE_PRODUITS_PARAMETRE):
    ...
    RESULTAT = []   # liste vide qui contiendras les informations du nouveau vente / JOUE LE ROLE D'INTERMEDIAIRE
    ...
    if ETAT_STOCK == 0:
        ...
        for PRODUIT in LISTE_PRODUITS_PARAMETRE:
            ...
            if  PRODUIT[1].lower() == NOM_PRODUIT.lower() and int(PRODUIT[2]) >= 0   and int(PRODUIT[3]) >= int(QUANTITE_VENDU) :  # Permet de verifier si le nom de produit vendu est bien dans la liste de produis dispos et si la quantité saisie par l'utilisateur ne pas superier a seul du produit ou un nombre négatif
                COMPTEUR +=1
                ...
                PRIX_TOTAL_A_PAYER = float(int(QUANTITE_VENDU)* PRODUIT[2])
                RESULTAT.append(id)
                RESULTAT.append(NOM_CLIENT.capitalize())
                RESULTAT.append(NOM_PRODUIT)
                RESULTAT.append(int(QUANTITE_VENDU))
                RESULTAT.append(PRODUIT[2])
                RESULTAT.append(PRIX_TOTAL_A_PAYER)
                RESULTAT.append(date)
                RESULTAT.append(heure)
                PRODUIT[3] = (PRODUIT[3] - int(QUANTITE_VENDU))
